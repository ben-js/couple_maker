// 모든 유저의 비밀번호를 1q2w3e4r로 업데이트하는 스크립트
...
async function updateAllPasswords() {
  try {
    ...
    console.log('🔐 모든 유저의 비밀번호를 업데이트 시작...');
    ...
    for (const user of usersResult.Items) {
      try {
        ...
        console.log(`✅ ${user.email} 비밀번호 업데이트 완료`);
      } catch (error) {
        console.error(`❌ ${user.email} 비밀번호 업데이트 실패:`, error);
      }
    }

    console.log('🎉 모든 유저의 비밀번호 업데이트 완료!');
    console.log('🔐 새로운 비밀번호:', password);

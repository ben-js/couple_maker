...
### 타입 안전성 및 any 사용 지양 (MUST FOLLOW)
- 타입스크립트 any 사용은 최대한 지양한다.
- 타입 단언(as), keyof, 타입 가드 등 타입 안전한 방식으로 해결할 것.
- 불가피하게 any를 쓸 경우 반드시 코드 주석으로 이유를 명시할 것.
- 타입 안전성은 코드 품질과 유지보수성의 핵심이다.

# 타입/인터페이스 관리 규칙
- 모든 도메인 데이터 타입/인터페이스는 반드시 src/types/ 하위에 파일로 분리해 정의한다.
- 화면, 서비스, 컴포넌트 등에서 타입이 필요할 때는 반드시 해당 타입 파일에서 import해서 사용한다.
- 타입/인터페이스가 변경될 경우, 관련 파일만 수정하면 전체 코드에 일관성 있게 반영된다.

## 🧑‍💻 SOLID 원칙 (필수 커서룰)

- **S: 단일 책임 원칙(Single Responsibility Principle)**
  - 한 파일/클래스/함수/컴포넌트/서비스/유틸/타입은 하나의 역할만 담당해야 함
- **O: 개방-폐쇄 원칙(Open/Closed Principle)**
  - 확장에는 열려 있고, 변경에는 닫혀 있어야 함(새 기능/타입/컴포넌트 추가는 기존 코드 수정 없이 가능)
- **L: 리스코프 치환 원칙(Liskov Substitution Principle)**
  - 상위 타입(인터페이스/타입)으로 하위 타입(구현체)을 대체해도 동작 보장
- **I: 인터페이스 분리 원칙(Interface Segregation Principle)**
  - 불필요한 의존성/메서드 없는, 목적별 인터페이스 분리
- **D: 의존성 역전 원칙(Dependency Inversion Principle)**
  - 구체 구현이 아닌 추상(인터페이스/타입)에 의존

> **모든 서비스/핸들러/컴포넌트/유틸/타입/테스트는 SOLID 원칙을 따라 설계/구현/리팩토링해야 한다.**

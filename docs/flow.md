...
### 🔹 \[11-1] 매칭 자동 만료(만료 처리) 정책

```
[자동 만료(만료 처리)란?]
- 사용자가 매칭 신청 후 일정 기간(예: 7일) 동안 응답(일정/장소 선택 등)이 없거나, 매칭이 진행되지 않을 때 시스템이 자동으로 해당 매칭을 만료(실패) 처리하는 기능입니다.

[자동 만료가 필요한 이유]
- 유령 매칭/방치 방지: 응답 없는 매칭이 계속 남아 시스템 리소스 낭비 및 다른 유저의 기회 저해 방지
- 공정한 기회 제공: 일정 기간 후 자동 만료로 다른 유저가 새로운 매칭을 시도할 수 있게 함
- 포인트/리소스 회수: 매칭 실패 시 자동으로 포인트 반환
- 데이터 정합성/관리: 불필요한 진행 중 데이터 정리, 통계/운영 효율성 향상
- 운영 자동화: 수동 확인/처리 없이 시스템이 자동으로 만료 처리

[동작 방식]
- 매칭 신청 후 7일간 응답(일정/장소 선택 등)이 없으면 상태를 failed로 변경
- 포인트 자동 반환, 이력 백업, 필요시 알림 발송
- finished 상태 3일 후 자동 삭제(개인정보 보호)

[예시]
- user-1이 매칭 신청 후 7일간 아무 응답이 없으면 MatchingRequests 상태를 failed로 변경, 포인트 반환, 이력 저장
- finished 상태가 된 매칭은 3일 후 자동 삭제 및 MatchingHistory에 백업
```
